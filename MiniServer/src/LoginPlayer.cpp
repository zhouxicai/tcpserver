...
CLoginPlayer::CLoginPlayer(CConnectManager* pConnectManager, LoggerPtr Logger)
{
...
bool    CLoginPlayer::ProcessInput()
{
	...
	if (nReceived <= 0)
	{
		...
		LOG4CXX_INFO(logger, "  Client Disconnected Actively...Host: "<<m_pSocket->m_Host);
...
//#pragma message("注意断连的时候会晤号不能清空，只能在收到game服的断连消息之后会晤号才能清空");
	...
	LOG4CXX_INFO(logger, "Accept Data Length: "<<nReceived<<"Host: "<<m_pSocket->m_Host<<"Port: "<<m_pSocket->m_Port);
	LOG4CXX_INFO(logger, m_RecvBuff);
...
//#pragma message("彭渊2012.04.27:这里有时候不能处理包是因为合包发送导致下面的判断语句出错了。修改下再测试")
	LOG4CXX_INFO(logger, "CLoginPlayer::ProcessPackets() m_nRecvLength: "<<m_nRecvLength<<" nPacketLength: "<<nPacketLength);
	...
	while(nPacketLength <= m_nRecvLength && nPacketLength > 0)
	{
		...
		LOG4CXX_INFO(logger, "Receive Command from Host: "<<m_pSocket->m_Host<<" Port: "<<m_pSocket->m_Port<<" Command ID: "<<nCommandID<<" dwUin:"<<dwUin);
		...
		case MSG_ID_REQUEST_CREATE_CHARACTER:
			...
			break;
		default:
			LOG4CXX_ERROR(logger, "Receive Undefined Command ID: "<<nCommandID<<"From Host: "<<m_pSocket->m_Host<<" Port: "<<m_pSocket->m_Port);
			break;
		}
		...
		if (m_nRecvLength < 0)
		{
			LOG4CXX_ERROR(logger, "Process Command Error, From IP: "<<m_pSocket->m_Host<<" Port: "<<m_pSocket->m_Port<<" Command ID: "<<nCommandID);
		}
...
{
	LOG4CXX_INFO(logger, "Begin Process Login Message...");
	...
	if (nRet != S_OK)
	{
		LOG4CXX_ERROR(logger, "Database Query Error Error Code: "<<nRet);
...
//	MSG_HEAD msgHead;
	...
	if (responseLogin.dwUin != INVALID_PLAYER_UIN)
	{
		...
		if (GetPlayerBaseInfo(m_dwUin, m_PlayerBaseInfo))
		{
			...
			LOG4CXX_INFO(logger, szSendBuffer);
	...
	LOG4CXX_INFO(logger, "Login Message Process End...");
...
int     CLoginPlayer::ProcessRegister(const char* szBuffer)
{
	...
	if (nRet != S_OK)
	{
		LOG4CXX_ERROR(logger, "Database Query account error, user_name="<<requestRegister.szUserName);
	}
	...
	if (nRows == 0)
	{
		...
		if (nRet != S_OK)
		{
			LOG4CXX_ERROR(logger, "Database query Max Uin error...");
		}
		...
		if (nRet != S_OK)
		{
			LOG4CXX_ERROR(logger, "insert new user into account table error, player_name="<<requestRegister.szUserName<<" ErrorID: "<<nRet);
		}
		else
		{
			//插入成功，开始创建玩家信息等数据结构
...
void    CLoginPlayer::WritePlayerItemInfo()
{
	...
	if (nRet != 0)
	{
		LOG4CXX_INFO(logger, "udpate database error!");
	}
}
...
void    CLoginPlayer::InitNewPlayerInfo(unsigned int dwUin, const char* szName)
{
	...
	if (nRet != S_OK)
	{
		LOG4CXX_ERROR(logger, "InitNewPlayerInfo Uin="<<dwUin<<" insert into base_info table error, error id="<<nRet);
	}
	...
	if (nRet != S_OK)
	{
		LOG4CXX_ERROR(logger, "InitNewPlayerInfo Uin="<<dwUin<<" insert into item_info table error, error id="<<nRet);
	}
